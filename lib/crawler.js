    ...
    self.onContent = function (error, toQueue, response, fromCache) {
        ...
        if (error) {
            ...
            if (toQueue.retries) {
                ...
                toQueue.callback(error,null,null,toQueue.context);
            }
        ...
        if (isHTML && toQueue.jQuery && toQueue.method!="HEAD") {
            ...
            try {
                ...
                toQueue.callback(null,response,$,toQueue.context);
                release(toQueue);
                response.body = null;
            } catch (e) {
                toQueue.callback(e,null,null,toQueue.context);
                release(toQueue);
            }
            
        } else {

            toQueue.callback(null,response,null,toQueue.context);
            release(toQueue);
        }
